...
2. датой первого git-коммита, добавившего файл (дата публикации, а не
   ...
   заметку в "последние");
3. mtime файла — например, при сборке вне репозитория или на
   shallow-клоне (см. `_git_first_commit_dates`: shallow-клон
   определяется явно и не используется, чтобы не подставлять неверную
   дату).
...
def _git_first_commit_dates(repo_root: Path) -> dict[str, float]:
    ...
    if (repo_root / ".git" / "shallow").exists():
        ...
        # --unshallow` в build command — см. README) у самого старого
        # видимого коммита нет родителя, и git считает его "первым"
        # коммитом для ВСЕХ файлов, что в нём есть, — включая файлы,
        # которые на самом деле были добавлены намного раньше. Такая
        # дата хуже, чем её отсутствие: лучше уйти на mtime, чем
        # уверенно показать неверную дату.
        return {}
    try:
