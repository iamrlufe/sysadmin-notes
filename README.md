...
## Если push стал долгим

Git хранит новые объекты по одному файлу (loose objects) и упаковывает их
в pack-файл только при сборке мусора. Пока сборки не было, `git push`
перебирает эти файлы по одному, и на macOS с антивирусом/индексатором
каждое открытие файла стоит дорого — на 599 объектах перебор занимал
больше минуты при том, что данных всего пара мегабайт.

Лечится штатной упаковкой:

```bash
git gc
```

599 объектов на 1.8 МБ сжались в один pack на 289 КБ. Повторять имеет
смысл раз в несколько десятков коммитов; `git gc --auto` сам срабатывает
только на 6700 объектах, то есть в этом репозитории почти никогда.

Вторая причина — размер истории: её целиком тянет каждая сборка на
Cloudflare (`git fetch --unshallow`). Поэтому в `.gitignore` не стоит
пускать в индекс регенерируемые артефакты, в первую очередь
датированные снимки `graphify-out/20*/`.
...
2. В начале файла указать теги (и по желанию описание для главной):
   ...
   без `section_label` — из названия раздела, без `date` — из даты первого
   git-коммита, добавившего файл (правки статьи дату не сдвигают). Заметка
   появится в «Последних заметках» автоматически.
